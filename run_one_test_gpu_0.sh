...
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES=1 poetry run python src/run_test.py \
  ...
  --nets google/siglip-base-patch16-224 google/efficientnet-b3 google/vit-base-patch16-224 microsoft/resnet-50 \
  --batch_size 512 \
  --num_workers 0

echo "Teste concluído para CLIP. Verifique os resultados e ajuste os parâmetros conforme necessário."